...
namespace Poker1
{
    public partial class MainPage : ContentPage
    {
        ...
        {
            base.OnAppearing();
            playerName = await PromptForPlayerName();
            await DisplayAlert("Welcome", $"Hello, {playerName}!", "OK");
        }

        private async Task<string> PromptForPlayerName()
        {
            string name = await DisplayPromptAsync("Welcome", "Enter your name:");
            while (string.IsNullOrWhiteSpace(name))
            {
                await DisplayAlert("Error", "Name cannot be empty. Please enter your name to proceed.", "OK");
                name = await DisplayPromptAsync("Welcome", "Enter your name:");
            }
            return name;
        }
